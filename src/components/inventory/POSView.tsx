...
import { Search, ShoppingCart, Package, Trash2, Plus, Minus, CreditCard, Banknote, Zap, Edit3, CheckCircle2 } from 'lucide-react';
...
const POSView: React.FC<POSViewProps> = ({ products, categories, formatCurrency, onSaleComplete }) => {
    ...
    }, [products, searchQuery, activeFilter, activeTypeFilter]);
    ...
    const addToCart = (product: Product, customPriceValue?: number) => {
        ...
        if ((product.type === 'physical' || !product.type) && product.stock === 0) return;
        ...
        setCart(prev => {
            ...
            if (existingIndex >= 0) {
                ...
                if ((product.type === 'physical' || !product.type) && existing.quantity >= product.stock) return prev;
                const newCart = [...prev];
    ...
    const updateQuantity = (productId: string, price: number, delta: number) => {
        setCart(prev => prev.map(item => {
            if (item.product.id === productId && item.product.price === price) {
                ...
                if (isPhysical && newQuantity > item.product.stock) return item;
    ...
    const handleFinalizeSale = async () => {
        ...
        try {
            // 1. Create the sale record (schema: total_amount, payment_method, operator)
            ...
            // 2. Create sale_items and update stock for each cart item
            for (const item of cart) {
                // Schema: sale (relation), product (relation), quantity, unit_price
                ...
                // 3. Deduct stock for physical products
                const isPhysical = item.product.type === 'physical' || !item.product.type;
                if (isPhysical) {
            ...
            // 4. Success: clear cart and show notification
            setCart([]);
            setSuccessMessage(`Venta de ${formatCurrency(total)} registrada exitosamente`);
            setTimeout(() => setSuccessMessage(null), 4000);

            // 5. Notify parent to refresh product data (updated stock)
            onSaleComplete?.();

        } catch (error: any) {
            console.error('[handleFinalizeSale] Error al procesar la venta:', error);
            if (error?.response?.data) {
                console.error('[handleFinalizeSale] Detalle de validación:', JSON.stringify(error.response.data, null, 2));
            }
    ...
    const getStockBadge = (product: Product) => {
        ...
        if (product.min_stock && product.stock <= product.min_stock) {
            return <span className="px-2 py-1 rounded-full text-xs font-bold border bg-orange-500/90 text-white border-orange-400/50 animate-pulse">{product.stock} un.</span>;
        }
        return <span className="px-2 py-1 rounded-full text-xs font-bold border bg-green-500/90 text-white border-green-400/50">{product.stock} un.</span>;
    };
                ...
                <div className="flex p-1 bg-slate-900/50 rounded-xl border border-white/5">
                    {typeFilterOptions.map(opt => (
                        <button
                            ...
                            className={`flex-1 py-2 text-sm font-medium rounded-lg transition-all ${activeTypeFilter === opt.key ? 'bg-slate-800 text-white shadow-sm' : 'text-slate-500 hover:text-slate-300'}`}
                        >
                            {opt.label}
                        </button>
                    ))}
                ...
                <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
                    {categories.map(cat => (
                        <button
                            ...
                            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors whitespace-nowrap ${activeFilter === cat
