...
export const WorkstationSetup: React.FC = () => {
    ...
    return (
        <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4">
            <Card className="w-full max-w-md bg-slate-900 border-white/5 shadow-2xl rounded-3xl relative overflow-hidden">
                ...
                <CardContent className="p-8">
                    ...
                    {error ? (
                        <div className="p-4 bg-red-500/10 border border-red-500/20 rounded-xl text-red-400 text-sm text-center">
                            {error}
                            <Button variant="link" onClick={() => window.location.reload()} className="mt-2 text-red-300 hover:text-white font-medium block w-full p-0">Reintentar</Button>
                        </div>
                        ...
                        <div className="space-y-3">
                            {workstations.map(ws => (
                                <Button
                                    ...
                                    className="w-full h-auto flex items-center p-4 bg-slate-800/50 hover:bg-slate-800 border-2 border-transparent hover:border-blue-500/30 rounded-2xl transition-all duration-300 group"
                                >
                                    <div className="flex-1 text-left flex flex-col">
                                        <h3 className="text-white font-medium">{ws.name}</h3>
                                        <p className="text-xs text-slate-500 mt-0.5 uppercase tracking-wider font-normal">TIPO: {ws.type}</p>
                                    </div>
